...
from z3c.schema2xml import serialize, serialize_to_tree, deserialize, DeserializationError
...
class ProductFolderRest(grok.REST):
	...
	try:
	    ...
	    for field, (exception, element) in e.field_errors.items():
                snippet = getMultiAdapter((exception, self.request, None, field, self, content), IErrorViewSnippet)
                snippet.update()
                error = etree.Element('error', field=field.__name__, message=snippet.render())
